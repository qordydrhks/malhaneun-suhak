...
- `{"at":"A","to":["B","C"],"label":"40°"}` — ⚠️ **label 을 안 주면 좌표로 잰 실제 각도 숫자가 찍힌다.** 숫자 없이 표시만 하려면 `"label": false`.
- 직각: `{"at":"H","to":["A","B"]}` 를 `rightAngles` 에.

### labels (글자)
- `{"point":"A","text":"A(1, -4)","dx":6,"dy":-6}` — `dx`/`dy` 는 화면 픽셀(y 는 아래로 +).
- 점 이름 없이 아무 자리에: `{"at":[2,3],"text":"여기","dot":false}`
- 점 찍기 기본값: `point` 로 붙이면 **점이 찍힌다**(`dot:true`), `at` 으로 붙이면 **점이 안 찍힌다**(`dot:false`).
  `at` 자리에 점을 찍고 싶으면 `{"at":[x,y],"text":"","dot":true}`.
- `onCircle` 의 `deg` 는 화면에서 시계 반대 방향(0°=오른쪽, 90°=위).
...
### 벤 다이어그램 (도형 좌표, `axes` 없음)
- 전체집합 U = `shapes` 직사각형 (`{"points":["U1","U2","U3","U4"]}`), 왼쪽 위 모서리에 `labels` 로 "U".
- 집합 = `circles` + 반투명 색: `{"center":[210,200],"r":110,"color":"#185FA5","fill":"#185FA5","fillOpacity":0.15}`.
  두 번째 원은 `#D85A30`. 겹치는 부분은 두 색이 섞여 저절로 진해진다.
- ⚠️ **한 영역만 골라 칠할 수 없다.** 대신 그 영역 한가운데에 `labels`(`at`) 로 "A−B", "A∩B" 같은 **글자를 크게** 넣고,
  caption 에 "○○ 부분이 A−B" 라고 말로 짚어라. 원소 개수 개념이면 영역마다 개수·원소를 적어라.
- 원 이름 A·B 는 원 바깥 위쪽에 붙여 영역 글자와 헷갈리지 않게.
