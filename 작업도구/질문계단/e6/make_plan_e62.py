# spec_e62.py → review/plan_e6-2.js (질문 고르기 분류안, 분류안 하나짜리 파일)
import json, io, os, sys
sys.stdout.reconfigure(encoding='utf-8')
HERE = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.abspath(os.path.join(HERE, '..', '..', '..'))
sys.path.insert(0, HERE)
from spec_e62 import S
E = json.load(io.open(os.path.join(ROOT, '작업도구', '회차', 'export', 'e6-2_blank.json'), encoding='utf-8'))
by_small = {}
for i in E['items']:
...
for key, sm in sorted(by_small.items()):
    ...
    for lid in spec:
        if lid.startswith('+'):
            r, q, a, k = spec[lid]
            rec = {'id': '%s:qa22e62%s%s' % (prefix, key[1], lid[1:]), 'big': sm['big'], 'small': sm['small'], 'kind': 'add',
...
body = ('/* 질문 고르기 — 기본으로 실어 두는 분류안 (초6-2)\n'
        '   Claude 분류(2026-09-22): 기준표_초등_질문.md 1~11절 · 초5 마스터 검토 기준 그대로. 원천 작업도구/질문계단/e6/spec_e62.py */\n'
        "(window.QR_BASE_PLANS = window.QR_BASE_PLANS || []).push({ key:'e6-2_2026-09-22b', data:" + json.dumps(plan, ensure_ascii=False, indent=0) + '});\n')
io.open(os.path.join(ROOT, 'review', 'plan_e6-2.js'), 'w', encoding='utf-8', newline='').write(body)
