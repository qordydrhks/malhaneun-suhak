# -*- coding: utf-8 -*-
"""고1-1 6조각(E1~E6)을 합쳐 기계 점검 + 사람이 읽을 시트를 만든다."""
import io, json, os, re, unicodedata

SP = os.path.dirname(os.path.abspath(__file__))
import sys
CH = ['E1', 'E2', 'E3', 'E4', 'E5', 'E6']
...
BAD_TERM = ['삼각함수', '라디안', '지수함수', '로그', '수열', '귀납법', '미분', '적분',
            '극한', '확률', '벡터', '명제', '역함수', '합성함수', '유리함수', '무리함수',
            '원의 방정식', '평행이동', '대칭이동', '내분점', '외분점', '벤다이어그램', '여집합', '부분집합']
...
TAIL = re.compile(r'설명해\s*봐|말해\s*봐|말해\s*줘|설명해\s*줘|어떻게|방법을|방법|과정을|과정|무엇일까|무엇인지|뭘까|일까|할까|까요')


def jac(a, b):
    a, b = TAIL.sub(' ', a), TAIL.sub(' ', b)
    A, B = toks(a), toks(b)
...
for it in out:
    ...
    for q in qs:
        ...
        if re.search(r'[PC][a-z0-9]*/[^(]', both):
            problems.append('nPr/r! 꼴 — 분모를 괄호로 (nPr/(r!)) — %s' % small[:24])
        for mm in re.finditer(r'\(([^()]*)\)', both):
            inner = mm.group(1)
            if ' ' in inner.strip() and re.search(r'\S/|/\S', inner) and re.search(r'\d\s+-?\d', inner):
                problems.append('행렬 / 양옆 공백 없음 — %s :: %s' % (small[:24], inner[:30]))
        if '너가' in both:
...
io.open(os.path.join(SP, 'sheet_h11.txt'), 'w', encoding='utf-8').write(chr(10).join(sheet))
io.open(os.path.join(SP, 'problems_h11.txt'), 'w', encoding='utf-8').write(chr(10).join(problems))
print('개념 %d / 원천 %d / 문항 %d' % (len(out), len(src), qtotal))
print('유형', json.dumps(tcnt, ensure_ascii=False))
print('---- 문제 %d건 (problems_h11.txt) ----' % len(problems))
